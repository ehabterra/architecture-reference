  ...
  - q: "In OAuth2, what is the access token for, versus an OIDC ID token?"
    options:
      ...
      - text: "Both are only for machine-to-machine"
        correct: false
    explain: "OAuth2 grants API access but not identity; OIDC adds an identity layer (the openid scope and an ID token). An ID token's audience is the client app — it tells the client who the user is — so it must never be used as an access token for your API."
...
Never use an ID token as an access token (its audience is the client and it only conveys who the user is, not what they may access), and don't roll your own identity layer — use a provider that supports OIDC. Always send tokens over HTTPS, and prefer in-process-validatable JWTs over per-request DB lookups.
</Callout>
